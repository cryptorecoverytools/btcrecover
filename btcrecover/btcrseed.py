        ...
        if not mpk and not addresses and not hash160s:
            ...
            while True:
                if tk_root:  # Skip if TK is not available...
                    ...
                    print("No addresses specified for search, generation only mode is used...")
                    addresses = [""]
#                    exit()
...
#                print("Loaded", len(self._known_hash160s), "addresses from database ...")
      ...
      with open('/tmp/generated_addresses.txt', 'a', encoding='utf-8') as ff:
        for count, mnemonic_ids in enumerate(mnemonic_ids_list, 1):
            ...
            for derived_seed, salt in _derive_seed_list:
                ...
                if self._verify_seed(seed_bytes, salt, output_file=ff):
                    return mnemonic_ids, count  # found it
    ...
    def _verify_seed(self, arg_seed_bytes, salt = None, output_file = None):
        if salt is None:
